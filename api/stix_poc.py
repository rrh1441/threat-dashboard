...
def get_all_flashpoint_vulnerabilities(params):
    ...
    if not FP_API_KEY: return {"error": "FP_API_KEY not configured."}
    if not FP_VULN_API_URL: return {"error": "FP_VULN_API_URL not configured."}
    headers = {"Authorization": f"Bearer {FP_API_KEY}", "Accept": "application/json"}
    api_url = f"{FP_VULN_API_URL}/vulnerabilities"; all_vulnerabilities = []; current_page = 0;
    page_size = FP_API_PAGE_SIZE; total_hits = None; max_pages = 100
    print(f"Starting fetch. URL: {api_url}, Params: {params}")
    while True:
        page_params = params.copy(); page_params['from'] = current_page * page_size; page_params['size'] = page_size
        print(f"Querying page {current_page + 1} (size={page_size})..."); response = None
        try:
            ...
            if page_vulnerabilities is None: page_vulnerabilities = data.get('data', [])
            if not isinstance(page_vulnerabilities, list): page_vulnerabilities = []
            ...
            num_returned = len(page_vulnerabilities); print(f"-> Got {num_returned} results.")
            if total_hits == 0: break
            if total_hits is not None and len(all_vulnerabilities) >= total_hits: break
            if data.get("next") is None and num_returned > 0: break # Stop if API says no more pages explicitly
            if num_returned < page_size: break # Stop if last page was not full
            if current_page >= max_pages -1 : print(f"Warn: Max pages ({max_pages}) reached."); break
            current_page += 1
        # Keep detailed exception handling
        except requests.exceptions.Timeout: error_msg = f"API Timeout querying Flashpoint page {current_page + 1}."; print(f"Error: {error_msg}"); return {"error": error_msg}
        except requests.exceptions.HTTPError as e: error_detail = f"{e.response.status_code}: "; try: error_detail += e.response.text[:200]; except Exception: error_detail += "(No body)"; print(f"Error: HTTP Error page {current_page + 1}: {error_detail}"); return {"error": f"API HTTP Error page {current_page + 1}: {error_detail}"}
        except Exception as e: print(f"Error: Unexpected pagination error: {traceback.format_exc()}"); return {"error": f"Unexpected pagination error: {e}"}
...
def handler(path):
    ...
    all_stix_objects = []; conversion_errors = 0
    for vuln_data in vulnerabilities:
        try:
            stix_objs_for_vuln = map_flashpoint_vuln_to_stix(vuln_data)
            all_stix_objects.extend(stix_objs_for_vuln)
        except Exception as map_err:
            conversion_errors += 1; vuln_id_err = vuln_data.get('id', 'UNKNOWN'); print(f"Error mapping ID {vuln_id_err}: {map_err}\n{traceback.format_exc()}")
